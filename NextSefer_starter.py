...
def check_first_run():
    ...
    try:
        # Django'yu başlat
        if not setup_django():
            logging.warning("Django kurulumu yapılamadı, ilk çalıştırma kontrolü atlanıyor")
            return
...
def show_message_box(title, message):
    ...
    try:
        # Windows'da çalışıyorsa MessageBox kullan (Tkinter'a bağımlılığı kaldır)
...
def open_browser_once():
    """Tarayıcıyı yalnızca bir kez açar"""
    global browser_opened
    
    # Thread güvenliği için lock kullan
    with browser_lock:
        if not browser_opened:
            try:
                # Sunucunun başlaması için 3 saniye bekle
                time.sleep(3)
                # Tarayıcıyı login sayfasına yönlendir
                logging.info("Web tarayıcısı açılıyor: http://localhost:8000/login/")
                webbrowser.open_new("http://localhost:8000/login/")
                browser_opened = True
            except Exception as e:
                logging.error(f"Tarayıcı açılırken hata oluştu: {str(e)}")
